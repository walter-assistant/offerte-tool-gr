...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const wanted = cleanPart(req.query?.projectNumber, '');
      if (!wanted) return res.status(400).json({ error: 'Projectnummer ontbreekt' });
      const token = await getAccessToken();
      const customers = await listFolders(token, BASE_PATH);
      for (const customer of customers) {
        const projects = await listFolders(token, customer.path_display || customer.path_lower);
        const match = findMatchingFolder(projects, wanted, true);
        if (!match) continue;
        const location = match.name.replace(new RegExp(`^${wanted.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*-?\\s*`, 'i'), '').trim();
        return res.status(200).json({ found: true, project: { projectNumber: wanted, customer: customer.name, location } });
      }
      return res.status(404).json({ found: false });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ error: err.message || 'Dropbox zoekfout' });
    }
  }
