...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!authRes.ok) {
      return sendJson(res, authRes.status, { error: data.error_description || data.msg || data.message || 'Inloglink mislukt.' });
    }

    return sendJson(res, 200, { ok: true });
  } catch (err) {
    console.error('magic-link error:', err);
    return sendJson(res, 500, { error: 'Inloglink mislukt: serverfout.' });
  }
}
